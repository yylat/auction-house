...
<div id="signUpModal" class="w3-modal pro-modal">
    <div class="w3-modal-content w3-card-4 w3-animate-zoom modal-content">
        ...
        <form id="signUpForm" class="w3-container bottom-padding" action="${pageContext.request.contextPath}/controller" method="post">
            ...
            <c:choose>
                <c:when test="${requestScope.usernameAlreadyExist == true}">
                    <div class="w3-container error-message">${usernameAlreadyExist}</div>
                </c:when>
                <c:when test="${requestScope.emailAlreadyExist == true}">
...
<c:if test="${requestScope.successfulRegistration == true}">
...
<c:if test="${requestScope.openSignUp == true}">
    <script>
